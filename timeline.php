...
echo '<div class="timeline-container" style="width:'.$width.';margin-top:100px;">';
echo '<div class="time-line-time">';
echo '<div id="timeline-line" style="width:'.$width.';"></div>';
for ($c0 = 1; $c0 <= $numyears; $c0++) {
    echo '<div class="time-line-circle" style="width:'.(100/$numyears).'%"><a class="time-line-href href'.$c0.'" href="#" onclick="change'.$c0.'()" id="time_click"  data-amount='.$data[year.$c0][amount].' data-price='.$data[year.$c0][price].' data-debt='.$data[year.$c0][amount-price].'><div class="circleLine"></div><span >'.$data[year.$c0][name].'</span></a></div>';
}
echo '</div>';
...
$(document).ready(function(){
	...
	$('.time-line-href').on('click', function(e){
		...
		prc=price;
		$('#figure').show(200);
		$('#prc').animateNumbers(parseInt(price), true, 1500);
		
		debt = amount - price;
		$('#fig').show(200);
		$('#debt').animateNumbers(parseInt(debt), true, 1500);
	});
	$('.timeline-circles').bind('click',function(){
		...
		$.get('?year='+yrs+'&table='+newInfo, function(data,status,xhr){
			$('#loading-cont').remove();
			$('#header').removeClass('loading');
			$('#timeline-outputtext').jOverlay({center:false,css:{position:'absolute',left:'10%', top:'100px'}});
			if(info=='Actual Expenditure'){
