...
for ($c0 = 1; $c0 <= $numyears; $c0++) {
    echo '<div class="time-line-circle" style="width:'.(100/$numyears).'%"><a class="time-line-href href'.$c0.'" href="#" onclick="change'.$c0.'()" id="time_click"  data-amount='.$data[year.$c0][amount].' data-price='.$data[year.$c0][price].'><div class="circleLine"></div><span >'.$data[year.$c0][name].'</span></a></div>';
}
echo '</div>';
...
$(document).ready(function(){
	...
	$('.timeline-circles').bind('click',function(){
		...
		$.get('?year='+yrs+'&table='+newInfo, function(data,status,xhr){
			...
			$('#timeline-outputtext').jOverlay({center:false,css:{position:'absolute',left:'10%', top:'700px'}});
